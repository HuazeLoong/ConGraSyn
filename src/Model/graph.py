...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x, dist, dist_bar: Tensor, mask=None, attn_bias=None, dist_embed=None):
        ...
        coverage_str = f">>> overall coverage range: {min_keep:.1f}% – {max_keep:.1f}%\n"
        ...
        if dist_embed is not None:
            ...
            v_msg = torch.matmul(attn, value.unsqueeze(2).expand(-1,-1,num_scales,-1,-1))
            msg   = v_msg + dist_msg
        else:
            msg = torch.matmul(attn, value.unsqueeze(2).expand(-1,-1,num_scales,-1,-1))


        # 最后沿 K 维拼到 head 维上
        msg = msg.transpose(1,2).contiguous().view(B, H*dist_bar.size(0), N_full, d_k)
...
class MultiScaleTransformer(nn.Module):
    ...
    def forward(self, x, dist, dist_bar, attn_mask, attn_bias=None, dist_embed=None):
        x, attn = self.self_attention(x, dist, dist_bar, mask=attn_mask, attn_bias=attn_bias, dist_embed=dist_embed)
...
class EncoderAtomLayer(nn.Module):
    ...
    def forward(self, x: Tensor, attn_mask: Tensor,
                attn_bias: Tensor = None, dist=None, dist_bar=None, dist_embed=None):
