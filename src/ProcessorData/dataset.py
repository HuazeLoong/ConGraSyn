...
class MyTestDataset(InMemoryDataset):
    ...
    def get_cell_feature(self, cellId): return self.testcell[self.cell2id[cellId]] if cellId in self.cell2id else False
    ...
    def convert_graph(graph_dict):
        ...
        data.edge_index = torch.tensor(graph_dict['edges'], dtype=torch.long).t().contiguous()
        ...
        # ===== 额外原子属性 =====
        for key in ['explicit_valence', 'formal_charge', 'num_explicit_Hs',
                    'hybridization', 'total_numHs', 'atom_is_in_ring']:
        ...
        feature_list = [
            data.atomic_num, data.chiral_tag, data.is_aromatic, data.degree,
            data.explicit_valence, data.formal_charge, data.num_explicit_Hs,
            data.hybridization, data.total_numHs, data.atom_is_in_ring
        ]
        ...
        data.FG_mask = torch.tensor(graph_dict.get('FG_mask', [0] * num_nodes), dtype=torch.bool)
        data.FG_edge_index = torch.tensor(graph_dict.get('FG_edge_index', []), dtype=torch.long).t().contiguous() \
            if graph_dict.get('FG_edge_index') else torch.empty((2, 0), dtype=torch.long)
    ...
    def process(self, xd, xd_fp, xd_smi, xd_mol, xd_graph, xt, y):
        ...
        if len(data_list) > 0:
            ...
            print("sample.perturbation:", type(sample.perturbation), sample.perturbation.shape)
