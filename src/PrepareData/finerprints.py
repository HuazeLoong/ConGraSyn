...
def condition_func2(mol, ring):
    is_saturated = all(
        mol.GetBondWithIdx(idx).GetBondType().name == "SINGLE" for idx in ring
    )
    is_aromatic = all(
        mol.GetBondWithIdx(idx).GetBondType().name == "AROMATIC" for idx in ring
    )
    all_carbon = all(
        mol.GetBondWithIdx(idx).GetBeginAtom().GetAtomicNum() == 6
        and mol.GetBondWithIdx(idx).GetEndAtom().GetAtomicNum() == 6
        for idx in ring
    )
...
def condition_func3(mol, ring):
    ...
    contain_N = any(
        mol.GetBondWithIdx(idx).GetBeginAtom().GetAtomicNum() == 7
        or mol.GetBondWithIdx(idx).GetEndAtom().GetAtomicNum() == 7
        for idx in ring
    )
...
def condition_func4(mol, ring):
    ...
    has_hetero = any(
        mol.GetBondWithIdx(idx).GetBeginAtom().GetAtomicNum() not in [1, 6]
        or mol.GetBondWithIdx(idx).GetEndAtom().GetAtomicNum() not in [1, 6]
        for idx in ring
    )
...
def calcPubChemFingerPart2(mol):
    ...
    mappings = [
        (
            "AtomRings",
            condition_func1,
            {3: 0, 4: 14, 5: 28, 6: 63, 7: 98, 8: 112, 9: 126, 10: 133},
        ),
        (
            "BondRings",
            condition_func2,
            {3: 1, 4: 15, 5: 29, 6: 64, 7: 99, 8: 113, 9: 127, 10: 134},
        ),
        (
            "BondRings",
            condition_func3,
            {3: 2, 4: 16, 5: 30, 6: 65, 7: 100, 8: 114, 9: 128, 10: 135},
        ),
        (
            "BondRings",
            condition_func4,
            {3: 3, 4: 17, 5: 31, 6: 66, 7: 101, 8: 115, 9: 129, 10: 136},
        ),
        (
            "BondRings",
            condition_func5,
            {3: 4, 4: 18, 5: 32, 6: 67, 7: 102, 8: 116, 9: 130, 10: 137},
        ),
        (
            "BondRings",
            condition_func6,
            {3: 5, 4: 19, 5: 33, 6: 68, 7: 103, 8: 117, 9: 131, 10: 138},
        ),
        (
            "BondRings",
            condition_func7,
            {3: 6, 4: 20, 5: 34, 6: 69, 7: 104, 8: 118, 9: 132, 10: 139},
        ),
    ]
